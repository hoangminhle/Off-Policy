...
class Simple_CNN_Model(TorchModelV2, nn.Module):
    def __init__(self, spec_tree, obs_space, action_space):
        conv_filters = spec_tree['conv_filters']
        ...
        input_layer = nn.Conv2d(3, conv_filters, kernel_size = 2, stride=1)
        ...
        num_output = 8*4*4
        layers.append(nn.Linear(num_output, fcnet_hiddens[0]))
        layers.append(activation_layer)
        ...
        value_layers.append(nn.Flatten())
        value_layers.append(nn.Linear(num_output, fcnet_hiddens[0]))
        value_layers.append(activation_layer)
