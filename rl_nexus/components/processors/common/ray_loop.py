...
from rl_nexus.components.models.Simple_AC_Model.Simple_AC_Model import Simple_AC_Model
...
class Model(Simple_AC_Model):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, model_spec_tree):
        super().__init__(model_spec_tree, obs_space, action_space)

# class MLP_Model(TorchModelV2, nn.Module):
#     def __init__(self, obs_space, action_space, num_outputs, model_config,
#                  name, proc_spec_tree, model_load_paths_dict, train_mode, original_obs_space):
#         TorchModelV2.__init__(self, obs_space, action_space, num_outputs,
#                               model_config, name)
    ...
    def __init__(self, processor):
        ...
        self.rllib_config={
            ...
            "model": {
                ...
                "custom_model_config": {
                    "model_spec_tree": proc_spec_tree['agent']['model']
                    # "fcnet_hiddens": proc_spec_tree['agent']['model']['fcnet_hiddens'],
                    ...
                    # "fcnet_activation": proc_spec_tree['agent']['model']['fcnet_activation'],
                    # "vf_share_layers": proc_spec_tree['agent']['model']['vf_share_layers']
                },
            },
            "framework": "torch",
            "min_iter_time_s": 0,
        ...
        # import pdb; pdb.set_trace()
        # (_, _, agent_spec_tree) = proc_spec_tree.get_component_info('agent')
        # model = agent_spec_tree.create_component('model', self.pr.environment.observation_space, self.pr.environment.action_space)
        ...
        # ModelCatalog.register_custom_model("simple_model", Simple_Model)
        
        # ModelCatalog.register_custom_model("mlp_model", MLP_Model)
        ModelCatalog.register_custom_model("simple_model", Model)
        ...
        if self.pr.train:
            ...
            while timesteps < self.pr.total_steps:
                ...
                if model_save_paths_dict is not None and metrics[0].currently_optimal:
                    ...
                    policy_save_tag += 1
                    trainer.get_policy().model.save_model_in_progress(model_save_paths_dict, policy_save_tag)
                    saved = True
