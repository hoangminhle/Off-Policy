...
class TDREG_Kernel(object):
    def __init__(self, dataset, obs_dim, act_dim, gamma, horizon,
        ...
        #* we only whiten over the non-terminal tuples?
        ...
        if self.input_mode == 'sa':
            ...
            #     self.x = torch.tensor(np.concatenate((self.obs,acts), axis=1), dtype=dtype)
            ...
            self.x = torch.tensor(self.obs, dtype=dtype)
    ...
    def prepare_torch_tensor(self):
        self.M = torch.tensor(self.M, dtype=dtype)
        self.alpha = torch.tensor(self.alpha, dtype=dtype)
        self.rews = torch.tensor(self.rews, dtype=dtype)
        self.rho = torch.tensor(self.rho, dtype=dtype)
        self.Phi_prime_pi = torch.tensor(self.Phi_prime_pi, dtype=dtype)
        self.Phi = torch.tensor(self.Phi, dtype=dtype)
        self.Phi_term_pi = torch.tensor(self.Phi_term_pi, dtype=dtype)
    ...
    def train(self, num_iter = 300, batch_size = 20000, td_ball_epsilon = 0.01, lr = 1e-3, w_reg =0.0, normalize_w = False,\
        ...
        Phi_0_pi = torch.tensor(np.mean(self.Phi_init_pi, axis=0)[:,None], dtype=dtype)
