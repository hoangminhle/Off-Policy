...
class MSWL(Basic_Alg):
    ...
    def build_graph(self):
        ...
        self.pr_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1])
        ...
        self.pr_ph_2 = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1])
    ...
    def create_density_ratio(self, obs_tf, *, factor=None, reuse=False, normalize=True):
        with tf.compat.v1.variable_scope(self.scope, reuse=reuse):
            ...
            w = tf.compat.v1.layers.dense(x, 1, activation=None, kernel_regularizer=tf.keras.regularizers.l2(0.5 * (1.)))
    ...
    def build_estimation_graph(self):
        ...
        self.value_estimation = tf.reduce_sum(input_tensor=T * rew) / tf.reduce_sum(input_tensor=T) / (1 - self.gamma)        
        # self.value_estimation = tf.reduce_sum(T * rew) /  (1 - self.gamma)        
    ...
    def create_loss_func(self):
        ...
        for index in range(len(Kernel)):
            ...
            self.loss += tf.squeeze(c * tf.matmul(tf.matmul(tf.transpose(a=x1), K), x2)) / sample_num
